...
export const WorkspacePage = () => {
  ...
  const handleRunAgent = async (e) => {
    ...
    try {
      const res = await fetch(`/api/agents/${COFOUNDER_AGENT_SLUG}/run`, {
        method: "POST",
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                                {runs.map((run) => (
                                    <div key={run.id} className="p-4 rounded-xl bg-slate-900/40 border border-slate-800 hover:border-emerald-500/30 transition-colors">
                                        <div className="flex justify-between items-start mb-2">
                                            <span className="text-xs text-slate-400">{new Date(run.created_at).toLocaleDateString()}</span>
                                            <span className={`text-xs px-2 py-1 rounded-full ${run.status === 'completed' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}>
                                                {run.status}
                                            </span>
                                        </div>
                                        <p className="text-sm text-slate-300 line-clamp-2">{JSON.parse(run.input_params).idea_summary}</p>
                                    </div>
                                ))}
                                {runs.length === 0 && (
